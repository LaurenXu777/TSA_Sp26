...
raw_inflow_data <- read.table(file="/Users/xxx/R/Time Series_R/TSA/Data/inflowtimeseries.txt",header=FALSE,skip=0)
...
pacf(ts_inflow_data,lag.max = 40)
...
#compute seas components
season_inflow_data<-array(0,nobs)
for (i in 1:nobs)
{season_inflow_data[i]<-beta_int+beta_coeff%*%dummies[i,]
}

#remove seas from series
deseason_inflow_data<-detrend_inflow_data - season_inflow_data

#convert to ts obj
ts_detrend_inflow_data<-ts(deseason_inflow_data,
                           start=c(1931,1),
                           frequency = 12)
ts_season<-ts(season_inflow_data,start = c(1931,1),frequency = 12)

autoplot(ts_inflow_data[,iHP])+
 #autolayer(ts_detrend_inflow, series="Detrend and Deseasoned Inflow")
  autolayer(ts_season,series="Seasonal Component")
```
